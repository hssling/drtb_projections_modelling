...
#' author: Takuya Yamanaka - Based on an original script by Philippe Glaziou in 2022
#' date: July 2023
#' output:
#'    html_document:
#'      toc: true
...
#' This script generates estimates of lives saved (with global and regional aggregates),
#' using ther GF estimation method.
#' 
#' The code is inelegant, slow, not optimized
#' (over 1 hour runtime on a macbook M2).
#' (with Ryzen 9 7950x, runtime is much shorter: around 15-20 min)
#' 
#' Deps: libraries data.table and here
#' 
#' 
#' Input: global.rda, regional.rda, est.rda
#' 
#' 
#' Output: 
#'   ~/lives_saved/output/global_aved.rda
#'   ~/lives_saved/output/HIVpos_global_saved.rda
#'   ~/lives_saved/output/regional_lives_saved.rda
#'   ~/lives_saved/output/country_lives_saved.rda
#'
library(data.table)
library(here)
library(dplyr)
...
i <- 2010
y <- c(2010, 2005, 2000) # data from 2005 and 2010 are required for section 2.3 of GTBR

#------------- looping start for 2005 and 2010 ---------------#

for (i in y) {
  
start_year = i # use 2010 for the table 2.3.1 and 2005 for texts
start_year_hiv = ifelse(start_year<=2004,2004,start_year) # previously 2004

load(here('inc_mort/analysis/global.rda')) 
global <- global %>% filter(year>=start_year)
load(here('inc_mort/analysis/regional.rda'))
regional <- regional %>% filter(year>=start_year)
load(here('inc_mort/analysis/est.rda'))
est <- est %>% filter(year>=start_year)
...
#        file = paste0(here('lives_saved/global_saved_'), Sys.Date(), '.csv'))

save(saved.global.print,file=here(paste0('lives_saved/output/global_saved_',start_year,'.rda')))


#' HIV+
global.lsaved <- lsaved(global[year > start_year_hiv])
global.lsaved <-
  global.lsaved[, list(
    year = as.character(year),
    savedn,
...
#        file = paste0(here('data/livessaved/HIVpos_global_saved_'), Sys.Date(), '.csv'))

save(hsaved.global.print,file=here(paste0('lives_saved/output/HIVpos_global_saved_',start_year,'.rda')))


#' by WHO region (2000-report_year-1)
#'
set.seed(101)
...
# fwrite(
#   saved.regional.print,
#   file = paste0(here('data/livessaved/regional_saved'), Sys.Date(), '.rda')
# )

save(saved.regional.print,file=here(paste0('lives_saved/output/regional_saved_',start_year,'.rda')))



#------- 

# do not run the script below every time as the process takes more than 1 hour!
...
cty.lsaved2 <- function(dta, start = start_year) {
  th <- 1000
...
out <- est[, cty.lsaved2(.SD, start = start_year), by = iso3]

# fwrite(out, file = paste0(here('csv/lsaved_'), Sys.Date(), '.csv'))
save(out,file=here('lives_saved/output/country_lives_saved.rda'))
