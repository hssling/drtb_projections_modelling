...
## e.g. once model chosen:
## model=$(<./utils/modelchoice.txt)
## R --slave --vanilla --args <2stanfitting.R $model
##
...
## NOTE debugging
## args <- c('separate_LerouxIntercept_2','-2','T')
## args <- c('separate_LerouxInterceptLerouxSlope_2')
## args <- c('together_LerouxInterceptLerouxSlopeQ_2')
...
## MCMC parms
niter <- 2e3 # keep short for comparison
nchains <- 2
cores <- 2
max_treedepth <- 10
options(mc.cores = parallel::detectCores())

if(!file.exists(here("drtb/outdata"))) dir.create(here("drtb/outdata"))

## NOTE 2025 sensitivity analysis
SA <- ".indSA" #NOTE set to "" for base case
## SA <- ""
## data
load(gh("drtb/data/isoidx5.Rdata"))
load(here("drtb/data/regkey.Rdata"))
load(gh("drtb/data/D4S{SA}.Rdata"))
...
## different data:
wphi <- c(
  "separate_global", "separate_globalH",
  "separate_regional"
)
...
save(samps,file=gh('~/Downloads/samps{variant}.Rdata'))
print(get_elapsed_time(samps) / 60)
## load(file=gh('stout/samps{variant}.Rdata'))

fn <- gh("drtb/plots/t.{variant}{omitcn}{omitTstr}{tallstring}{SA}.txt")
cat(get_elapsed_time(samps) / 60, file = fn)

## plot/save
maxyr <- 2000 + D4S$T - 1 ## compute max year from stan data
K <- getRRprops(samps, D4S$J)
save(K, file = gh("drtb/outdata/noomit_K.{variant}{SA}.Rdata"))
...
if (omitting) {
  if (!omittingT) {
    ...
    save(W, file = gh("drtb/outdata/W.{variant}.{omitcn}{SA}.Rdata"))
    K <- K[iso3 == omitcn]
  }
  fn <- gh("drtb/outdata/K{omitTstr}{tallstring}.{variant}.{omitcn}{SA}.Rdata")
  fn <- gsub("\\.\\.", "\\.", fn) # simplest ends up with ".."; removed for backwards consistency
  save(K, file = fn)
  if (tallstring == "TA") {
    KO <- makeplotdata(K)
    save(KO, file = gh("drtb/outdata/KOTTA.{variant}.{omitcn}{SA}.Rdata"))
  }
} else { # full inference

  KO <- makeplotdata(K, maxyr = maxyr)
  save(KO, file = gh("drtb/outdata/KO.{variant}{SA}.Rdata"))

  fn <- gh("drtb/plots/r.{variant}{SA}")
  ...
  fn <- gh("drtb/outdata/sdr.{variant}{SA}.csv")
  fwrite(sdr, file = fn)

  nmz <- names(samps)
  nmz <- nmz[!grepl("\\[", nmz)] # only top level things
  smy <- summary(samps, pars = nmz)
  save(smy, file = gh("drtb/outdata/smy.{variant}{SA}.Rdata"))

  loo.out <- loo(samps, save_psis = FALSE)
  save(loo.out, file = gh("drtb/outdata/loo.out.{variant}{SA}.Rdata"))

  ll <- extract_log_lik(samps)
  waic.out <- waic(ll)
  save(waic.out, file = gh("drtb/outdata/waic.out.{variant}{SA}.Rdata"))

  lls <- rowSums(ll)
  dll <- list(
    data.likelihood.mean = mean(lls),
    data.likelihood.sd = sd(lls)
  )
  save(dll, file = gh("drtb/outdata/dll.{variant}{SA}.Rdata"))
}
