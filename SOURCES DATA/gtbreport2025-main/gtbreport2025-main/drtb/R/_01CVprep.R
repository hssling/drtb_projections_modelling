...
CC[,ncns:=nrow(CC):1]
CC <- merge(CC,regkey[,.(iso3,g.whoregion=g_whoregion)],by='iso3')
...
omith <- 0.8
CC[pcdc>1-omith] #103 countries, 18 data points (A)
CC[pcdc>1-omith,table(g.whoregion)]
...
abline(h=0.8,col=6,lty=2);abline(v=103,col=6,lty=2)
dev.off()

## create & save worklists

c.omit.vA <- CC[pcdc>1-omith,iso3]
...
cat(c.omit.vAt,file=gh('drtb/data/c.omit.vAt.txt'))
