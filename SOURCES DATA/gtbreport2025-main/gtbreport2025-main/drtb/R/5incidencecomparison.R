...
for (modrt in rootlst) {
  ...
  fn <- gh("drtb/outdata/KO.{modrt}.Rdata")
  if (!file.exists(fn)) next # skip if not there
  load(fn)

  ## create incidence
  tout <- addIncidence(KO[year > 1999 & year <= ylimz[2]])
  ...
  GP <- ggplot(glob, aes(
    ...
    theme_light() +
    xlab("Year") +
    ylab("RR-TB as proportion of all incidence") +
    ggtitle(modrt)
  ## GP
