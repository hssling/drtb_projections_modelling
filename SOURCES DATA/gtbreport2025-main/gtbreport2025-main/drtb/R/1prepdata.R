...
## helper functions
source(gh("drtb/R/utils/CIhelpers.R")) # for binomial CIs
...
## make old-style HBC key
HBC <- data.table(
  iso3 = unique(grpmbr$iso3),
  g.hbmdr = FALSE, g.hbtbhiv = FALSE, g.hbc = FALSE
)
HBC[iso3 %in% grpmbr[group_type == "g_hb_tb", iso3], g.hbc := TRUE]
HBC[iso3 %in% grpmbr[group_type == "g_hb_tbhiv", iso3], g.hbtbhiv := TRUE]
HBC[iso3 %in% grpmbr[group_type == "g_hb_mdr", iso3], g.hbmdr := TRUE]
save(HBC, file = gh("drtb/data/HBC.Rdata"))
...
## load(gh("drtb/data/LB.indSA.Rdata"))
## load(gh("drtb/data/YB.indSA.Rdata"))
...
LB[
  case %in% c("dh0.x", "dh2.x"),
  c("RR.k", "RR.N") := .(dr_r_nh + mdr + xpert_dr_r, dst_rlt + xpert)
]
LR <- LB[, .(id, patients,
  type = "surveillance", iso3, year,
  case, RR.k, RR.N
)]
...
## surveys
YB[
  case %in% c("r", "r.hnr", "r.hr"),
  c("RR.mid", "RR.lo", "RR.hi") := .(rr_pcnt, rr_pcnt_lo, rr_pcnt_hi)
]
YB[
  case %in% c("dh0", "dh1.h", "dh0.x"),
  c("se1", "se2") := .(
    (dr_r_nh_pcnt_hi - dr_r_nh_pcnt_lo) / 3.92,
...
YB[
  case %in% c("dh0.x"),
  md := (se1 / se)^2 * md + (se2 / se)^2 * xpert_dr_r_pcnt
]
...
## ## NOTE 2025 sensitivity analysis
## save(RPD, file = here("drtb/data/RPD.indSA.Rdata"))
## load(file = here("drtb/data/RPD.indSA.Rdata"))
...
getmusig <- function(lo, mid, hi) {
  mid <- mid / 1e2
  lo <- lo / 1e2
  hi <- hi / 1e2 # convert pcnt to number
...
save(regkey, file = here("drtb/data/regkey.Rdata"))
...
LD[["L.new.dh0.N"]] <- as.matrix(tmp[, .(
  dst_rlt - dr_h_nr - dr_r_nh - mdr,
  dr_h_nr, dr_r_nh, mdr
)])
...
MS <- getmusig(
  tmp$xpert_dr_r_pcnt_lo, tmp$xpert_dr_r_pcnt,
...
tmp <- na.omit(tmp)
MS.hnr <- getmusig(
  tmp$dr_h_nr_pcnt_lo, tmp$dr_h_nr_pcnt,
  tmp$dr_h_nr_pcnt_hi
)
MS.rnh <- getmusig(
  tmp$dr_r_nh_pcnt_lo, tmp$dr_r_nh_pcnt,
  tmp$dr_r_nh_pcnt_hi
)
...
## ## NOTE 2025 sensitivity analysis
## save(D4S, file = gh("drtb/data/D4S.indSA.Rdata"))
