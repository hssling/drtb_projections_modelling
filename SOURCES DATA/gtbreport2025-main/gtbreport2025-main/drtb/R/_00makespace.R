...
if(file.exists(fn)){
  ...
  tb <- load_gtb('tb')
  print(isoidxL <- unique(tb$iso3)) #217
  if(!file.exists(here("dtb/data"))) dir.create(here("drtb/data"))
  save(isoidxL,file=here("drtb/data/isoidxL.Rdata"))
}
...
## WTB <- WTB[which(!(WTB@data$iso3 %in% c('ATA','VIR'))),]
WTB <- WTB[which(WTB@data$iso3 %in% isoidxL),] #drops BES (Bonnaire) and TKL (Tokelau)
dim(WTB)
tmp <- unique(WTB@data$iso3)
length(tmp) #213
setdiff(isoidxL,tmp) #"ANT" "GRL" "SCG" "TKL" (SCG=Serbia & Montenegro)
...
## changes for PAK
whop <- which(isoidx=='PAK')
who <- W[whop,]
who <- which(who>0)
isoidx[who]

## instead use: see https://en.wikipedia.org/wiki/Immigration_to_Pakistan
pakn <- c('AFG','BGD') ## 'IND', 'IRN','CHN')
who <- which(isoidx %in% pakn)

W[whop,] <- W[,whop] <- 0 #delete
W[whop,who] <- W[who,whop] <- 1

## check
who <- W[which(isoidx=='PAK'),]
who <- which(who>0)
isoidx[who] #OK
