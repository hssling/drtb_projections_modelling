...
#' Include subnational data from below
#' we will exclude other subnational data
subnationalkeepers <- c("PNG", "BRA", "CAF", "PRK") #NOTE PAK added below for RR
## TODO check
...
Hn[
  all_areas_covered_new == 1 | iso3 %in% subnationalkeepers,
  id := paste0(
    "N_", ifelse(source_new == "Survey", "Y", "L"),
    "_", iso3, "_", year_new
  )
]
Hn[, .(iso3, source_new, year_new, all_areas_covered_new, id)]
Hr[
  all_areas_covered_ret == 1 | iso3 %in% subnationalkeepers,
  id := paste0(
    "R_", ifelse(source_ret == "Survey", "Y", "L"),
    "_", iso3, "_", year_ret
  )
]
...
#' NOTE this is saved out below with H because removal of badids is synced (see next section)

#' # Rif-rated data
#'
#' Read in
## new
Rn <- load_gtb("drnew", convert_dashes = FALSE)

Rn[iso3 == "IND"] # NOTE this is already done

## ## NOTE 2025 sensitivity analysis: exclude 2024 data
## Rn <- Rn[!(iso3=='IND' & year_new==2024)]
## Rn[iso3=='IND']

## ret
Rr <- load_gtb("drret", convert_dashes = FALSE)

#' NOTE exclude 2022 India data point
Rr[iso3 == "IND"] # NOTE this is already done

## Rr <- Rr[!(iso3=='IND' & year_ret==2022)]
## Rr[iso3=='IND']

## ## NOTE 2025 sensitivity analysis: exclude 2024 data
## Rr <- Rr[!(iso3=='IND' & year_ret==2024)]
## Rr[iso3=='IND']

#' Also include Pakistan Sindh
subnationalkeepers <- c(subnationalkeepers, "PAK") # TODO check: "PNG" "BRA" "CAF" "PRK" "PAK"
...
#' Other country problems to drop:
R[iso3 == "GNB"] # 2022/3 already removed
badids <- c(
  R[(iso3 == "ZAF" & year == 2021), id], # V
  R[(iso3 == "COG" & year %in% 2022:2023), id], # 0 & 60%
  R[(iso3 == "GAB" & year == 2023 & patients == "ret"), id] # odd zero
)
R <- R[!id %in% badids]
H <- H[!id %in% badids] #NOTE remove from H also

R[iso3 == "GAB" & year == 2023 & patients == "ret"] #CHECK


#' Check Pakistan
R[iso3 == "PAK"][order(patients, year), .(country, year, source, patients)]
tmp <- R[iso3 == "PAK"]
tmp[grepl("Sindh", area_desc)]
tmp[year == 2019 & patients == "ret"]

#' Remove the Sindh data for ret as already in the national data:
R <- R[!(iso3=='PAK' & patients=='ret' & grepl('Sindh',area_desc))]

#' Check R2 extras exist:
R[iso3 == "BFA"] ## Burkina Faso 2023
R[iso3 == "BDI"] ## Burundi 2022
R[iso3 == "CIV"] ## Côte d'Ivoire 2023
R[iso3 == "MOZ"] ## Mozambique 2021 (updated, and year changed to 2021)
R[iso3 == "NPL"] ## Nepal 2023
R[iso3 == "NER"] ## Niger 2022
R[iso3 == "TGO"] ## Togo 2023
R[iso3 == "UZB"] ## Uzbekistan 2023
...
## ## NOTE 2025 sensitivity analysis
## save(R,file=here('drtb/data/R.indSA.Rdata'))
...
#' NOTE CHECK of above to ensure nothing sneaks back in via H
B[id %in% badids]
...
length(totden)
totden0 <- B[totden == 0, .(
  iso3, year, patients, id, dst_rlt, r_rlt, xpert,
  dr_h_nr, dr_r_nh, mdr,
  dst_rlt_hr, dst_rlt_rr,
  rr, xpert_dr_r
)]
...
YB[!is.na(mdr_pcnt), .N, by = .(
  is.na(mdr_pcnt_lo),
  is.na(mdr_pcnt_hi)
)] # OK

YB[!is.na(xpert_dr_r_pcnt), .N, by = .(
  is.na(xpert_dr_r_pcnt_lo),
  is.na(xpert_dr_r_pcnt_hi)
)] # OK

YB[!is.na(rr_pcnt), .N, by = .(is.na(rr_pcnt_lo), is.na(rr_pcnt_hi))] # OK

YB[!is.na(dst_rlt_hr_pcnt), .N, by = .(
  is.na(dst_rlt_hr_pcnt_lo),
  is.na(dst_rlt_hr_pcnt_hi)
)] # OK

YB[!is.na(dst_rlt_rr_pcnt), .N, by = .(
  is.na(dst_rlt_rr_pcnt_lo),
  is.na(dst_rlt_rr_pcnt_hi)
)] # OK
...
YB[!is.na(dr_h_nr_pcnt) & Hrated == "yes", .N,
  by = .(
    is.na(dr_h_nr_pcnt_lo),
    is.na(dr_h_nr_pcnt_hi)
  )
] #
...
YB[ !is.na(dr_h_nr_pcnt) &
     Hrated=='yes' &
     is.na(dr_h_nr_pcnt_lo) & is.na(dr_h_nr_pcnt_hi)] #


#' DROP YB (above)
YB[iso3 == "TUN" & year == 2012, dr_h_nr_pcnt := NA] #
...
mdnm
YB[!is.na(rr_pcnt), .N, by = .(
  is.na(xpert_dr_r_pcnt),
...
#' # Ad hoc changes 2023
#'
#' In order to reduce the influence of surveillance data relative to survey data in ZMB and PHL:
#'
load(file = here("drtb/data/LB.Rdata"))
load(file = here("drtb/data/YB.Rdata"))
...
## get the scaling to equivalize the mean variance for Y vs L
ymnvar <- YB[iso3 == "PHL", .(Vy = mean(((mdr_pcnt_lo - mdr_pcnt_hi) / 392)^2)),
  by = patients
]
lmnvar <- LB[iso3 == "PHL", .(Vl = mean((rr * (r_rlt - rr)) / r_rlt^3)),
  by = patients
]

mnvars <- merge(ymnvar, lmnvar, by = "patients")
mnvars[, VyoVl := Vy / Vl]
sn <- function(x) as.integer(x / mnvars[patients == "new"]$VyoVl)
sr <- function(x) as.integer(x / mnvars[patients == "ret"]$VyoVl)
mnvars #~90

## apply to RR data
LB[iso3 == "PHL" & patients == "new", c("rr", "r_rlt") := .(sn(rr), sn(r_rlt))]
LB[iso3 == "PHL" & patients == "ret", c("rr", "r_rlt") := .(sr(rr), sr(r_rlt))]

## LB[iso3=='PHL' & patients=='new',
##    c('mdr','dst_rlt','dst_rlt_hr','dst_rlt_rr','rr','r_rlt'):=
##      .(sn(mdr),sn(dst_rlt),sn(dst_rlt_hr),sn(dst_rlt_rr),sn(rr),sn(r_rlt))]
## LB[iso3=='PHL' & patients=='ret',
##    c('mdr','dst_rlt','dst_rlt_hr','dst_rlt_rr','rr','r_rlt'):=
##      .(sr(mdr),sr(dst_rlt),sr(dst_rlt_hr),sr(dst_rlt_rr),sr(rr),sr(r_rlt))]

LB[iso3 == "PHL"] # NOTE worry about rounding for smaller cats
...
## get the scaling to equivalize the mean variance for Y vs L
ymnvar <- YB[iso3 == "ZMB", .(Vy = mean(((rr_pcnt_lo - rr_pcnt_hi) / 392)^2, na.rm = TRUE)),
  by = patients
]
lmnvar <- LB[iso3 == "ZMB", .(Vl = mean((rr * (r_rlt - rr)) / r_rlt^3, na.rm = TRUE)), by = patients]
mnvars <- merge(ymnvar, lmnvar, by = "patients")
mnvars[, VyoVl := Vy / Vl]
sn <- function(x) as.integer(x / mnvars[patients == "new"]$VyoVl)
mnvars #~20
...
## apply to RR data
LB[iso3 == "ZMB" & patients == "new", c("rr", "r_rlt") := .(sn(rr), sn(r_rlt))]
LB[iso3 == "ZMB"] # NOTE worry about rounding for smaller cats

#' Ad hoc change to 2024 India surveillance data to reflect misclassification:
fac <- 1 #
fac.new <- 0.945
fac.ret <- 0.94
LB[iso3 == "IND" & year == 2024]
LB[
  iso3 == "IND" & patients == "new" &
    year == 2024, # dial down RR by fac
  c("mdr", "rr") := .(as.integer(fac.new * mdr), as.integer(fac.new * rr))
]
LB[
  iso3 == "IND" & patients == "ret" &
    year == 2024, # dial down RR by fac
  c("mdr", "rr") := .(as.integer(fac.ret * mdr), as.integer(fac.ret * rr))
]
...
## ## NOTE 2025 sensitivity analysis
## save(LB,file=here('drtb/data/LB.indSA.Rdata'))
## save(YB,file=here('drtb/data/YB.indSA.Rdata'))
