...
load(gh("drtb/data/LB{SA}.Rdata")) # surveiLLance
load(gh("drtb/data/YB{SA}.Rdata")) # surveY
...
nrow(LB) # 177
nrow(YB) # 28
intersect(LB$iso3, YB$iso3) # "LSO" "TGO"


## use H regardless of R: do we want MDR if missing?
YBR <- YB[
...
## LB <- LB[dst_rlt_hr<=dst_rlt]

## create binomial confidence intervals
LB[, c("prop.H", "prop.H.lo", "prop.H.hi") := MLH(dst_rlt_hr, dst_rlt)]
LB[, c("prop.H", "prop.H.lo", "prop.H.hi") := .(
  prop.H / 1e2,
  prop.H.lo / 1e2, prop.H.hi / 1e2
)]
LBR <- LB[, .(id, iso3, year, patients, prop.H, prop.H.lo, prop.H.hi)]


## any in both now?
(both <- intersect(LBR$iso3, YBR$iso3)) # "LSO" "TGO"
YBR[iso3 %in% both]
LBR[iso3 %in% both]


## NOTE typically much lower proportions in the surveillance data than survey
...
HP <- rbind(YBR, LBR)
nrow(HP)
HP[, length(unique(iso3))]


## regional meta-analyses - following PGs example
HP[, prop.H.sd := (prop.H.hi - prop.H.lo) / 3.92]
tmp <- est[year == yr, .(iso3, g.mdr = g.whoregion)]
FSU <- c(
  "ARM", "AZE", "BLR", "GEO", "KAZ", "KGZ",
  "MDA", "TJK", "TKM", "UKR", "UZB"
)
tmp[iso3 %in% FSU, g.mdr := "EEU"] #to match previous year MDR grouping
...
save(HP, file = gh("drtb/data/HP{SA}.Rdata"))
...
## save out as Rdata
HR_country <- KOH
attr(HR_country, "timestamp") <- Sys.Date() #set date
fn <- gh("drtb/output/HR_country{SA}.rda")
save(HR_country, file = fn)
...
## common location
attr(glob.H, "timestamp") <- Sys.Date() #set date
save(glob.H, file = gh("drtb/data/glob.H{SA}.Rdata"))

summary(glob)

## save out as Rdata
HR_global <- glob
attr(HR_global, "timestamp") <- Sys.Date() #set date
fn <- gh("drtb/output/HR_global{SA}.rda")
save(HR_global, file = fn)
