...
key <- est[year == yr, .(iso3, g.mdr = g.whoregion)]
FSU <- c(
  "ARM", "AZE", "BLR", "GEO", "KAZ",
  "KGZ", "MDA", "TJK", "TKM", "UKR", "UZB"
)
key[iso3 %in% FSU, g.mdr := "EEU"] #to match previous year MDR grouping
...
## reshape
FM <- melt(FDl,
  id = c("iso3", "g_whoregion", "year", "droprr", "droprrp", "dropmdr")
)
...
FM <- FM[, .(value = sum(value)),
  by = .(iso3, g_whoregion, year, type, frac)
] # NOTE a few repeats
...
## both types
FB <- rbind(FM, FDy)
save(FB, file = gh("drtb/data/FB{SA}.Rdata"))
...
for (reg in unique(FB[, g_whoregion])) {
  ...
  GP <- ggplot(
    ...
    rot45
  ggsave(GP, file = gh("drtb/plots/f_{reg}{SA}.pdf"), w = 12, h = 15)
}

## carry on
load(file = gh("drtb/data/FB{SA}.Rdata"))
...
cnt <- FB[!is.na(k + N), .(n = .N), by = iso3] # data count
...
FBR <- FBR[FQR.sd > 0] # odd single case
...
save(FQR, file = gh("drtb/data/FQR{SA}.Rdata"))
...
save(FQR_in_RR_country, file = gh("drtb/output/FQR_in_RR_country{SA}.rda"))


load(gh("drtb/output/db_dr_country{SA}.rda"))
...
save(FQR_in_RR_region, file = gh("drtb/output/FQR_in_RR_region{SA}.rda"))
...
save(FQR_in_RR_global, file = gh("drtb/output/FQR_in_RR_global{SA}.rda"))
