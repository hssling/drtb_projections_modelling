#' ---
#' title: PAF and derived attributable cases to 5 risk factors by country
...
#' 
#' Input: 
#'     - est.rda (TB estimates), sdg.rda and pop.rda 
#'     - incidence splits by age from Pete Dodd datasets in ~/disaggregation/output/data
#' 
#' 
...
#' 
#' Update 2025:
#'  - Diabetes prevalence was updated using the last GHO published estimates available for 2022
#'  - Update of the RR of diabetes as per 2024 systematic review
#'  
...
# load(here('disaggregation/output/A_country_incidence_disaggregated_age_sex_num.Rdata'))
...
temp<-db_estimates_country_all
temp<-temp[age_group %in% c("0-14","15plus","a") & year==yr,]

incs <- pivot_wider(
  data = temp,
  id_cols = c(iso3),
  names_from = c(age_group,sex),
  values_from = c(best,lo,hi)
)


# Function that returns the last non missing value of a vector or NA
#
lastv <-
  function(x)
...
# # Risk Ratios (mean, sd)
#
rr.alc <-  c(3.33, (5.19 - 2.14) / 3.92) # alcohol, Eur Resp Dis 2017
rr.dia <-  c(1.6, (1.80 - 1.42) / 3.92) # diabetes, MA review in 2024
...
incs2 <- as.data.frame(incs2)

incs3 <- with(incs2, {
  data.frame(
    iso3 = iso3,
    inc.m = best_a_m / pop * m,
    inc.m.sd = ((hi_a_m - lo_a_m) / 3.92) / pop * m,
    inc.f = best_a_f / pop.f * m,
    inc.f.sd = ((hi_a_f - lo_a_f) / 3.92) / pop.f * m,
    inc.15plus = best_15plus_a / adults * m,
    inc.15plus.sd = ((hi_15plus_a - lo_15plus_a) / 3.92) / adults * m,
    inc.15plus.m = best_15plus_m / adults.m * m,
    inc.15plus.m.sd = ((hi_15plus_m - lo_15plus_m) / 3.92) / adults.m * m,
    inc.15plus.f = best_15plus_f / adults.f * m,
    inc.15plus.f.sd = ((hi_15plus_f - lo_15plus_f) / 3.92) / adults.f * m,
    pop = pop,
    pop.m = pop.m,
    pop.f = pop.f,
    adults = adults,
...
### Merge new attributable cases based on undernutrition and diabetes

load(here("inc_mort/analysis/undernutrition_new_att.rda"))
load(here("inc_mort/analysis/diabetes_new_att.rda"))

malnutpaf[,sex:="a"]
diabetespaf[,sex:="a"]

#duplicates of sex variables to remove
att$sex=NULL
att$sex=NULL
att$sex=NULL
att$sex=NULL
...
att2=merge(att,malnutpaf,by=c("iso3","sex"),all.x=T)
att3=merge(att2,diabetespaf,by=c("iso3","sex"),all.x=T)
...
att[sex=='a', sums(inc.at.und.num.new)]

att[sex=='a', sums(inc.at.dia.num)]
att[sex=='a', sums(inc.at.dia.num.new)]
...
att2 <- att[, list(
  ...
  inc.at.dia.hi.num,
  inc.at.dia.num.new,
  inc.at.dia.lo.num.new,
  inc.at.dia.hi.num.new,
  inc.at.alc.num,
  inc.at.alc.lo.num,
  inc.at.alc.hi.num,
  inc.at.smk.num,
  ...
  inc.at.und.hi.num,
  inc.at.und.num.new,
  inc.at.und.lo.num.new,
  inc.at.und.hi.num.new,
  sex
)]
...
att8 <- melt(att2, id.vars = c(1,2, 19:20), measure.vars = 18)
att9 <- melt(att2, id.vars = c(1,2, 22:23), measure.vars =21 )

for (i in list(att3, att4, att5, att6, att7,att8,att9)) setnames(i, c('iso3', 'sex', 'lo', 'hi', 'risk.factor', 'best'))
att3[, risk.factor := 'hiv']
att4[, risk.factor := 'diabetes old']
att5[, risk.factor := 'diabetes']
att6[, risk.factor := 'alcohol']
att7[, risk.factor := 'smoking']
att8[, risk.factor := 'undernutrition old']
att9[, risk.factor := 'undernutrition']

rf <-
  rbind(att3, att4, att5, att6, att7,att8,att9)
rf[, measure := 'inc']
rf[, unit := 'num']
setkey(rf, iso3)
...
sel <- rf$risk.factor %in% c('hiv', 'undernutrition old',"undernutrition") & rf$sex %in% c('m','f')
table(sel)
rf <- rf[!sel]
