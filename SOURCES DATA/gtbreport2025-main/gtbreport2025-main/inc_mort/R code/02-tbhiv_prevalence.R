#' ---
#' title: HIV prevalence in TB
#' author: Mathieu Bastard, initial code from Philippe Glaziou
#' date: 03/06/2025
#' ---
...
# No more edits this year as compared to last year
...
# COG 2023 to investigate
#
sel <- tbhiv3$iso3 == 'COG' & tbhiv3$year == 2023
tbhiv3$tbhiv.routine[sel] <- NA
...
# sel=tbhiv$iso3=="CZE" & tbhiv$year==2022:yr
# tbhiv[sel, tbhiv:=NA]
...
for (i in wr) {
  p <-
    qplot(
      year,
      0,
      data = subset(tbhiv, g.whoregion == i & year>=2010),
      geom = 'line',
      colour = I('grey90')
    ) +
