pkgs.needed <- c(
  ## "MCMCpack",                       # for inverse wishart etc
  ## "lattice",                        # MCMC chain plots
  "readxl",                         # for reading excel
  "data.table",                     # data handling
  "ggplot2",                        # plotting
  "scales",                         # plotting
  "productplots",                   # for mosaic plots
  "gridExtra",                      # grouping plots
  "ggpubr",                         # plot arrangents
  "ggrepel",                        # plot labelling
  "glue",                           # strings
  "knitr",                          # for table formatting
  "stringr",                        # string searching
  "metafor"                         # for MA of sex splits in kids
)
...
## NOTE manually check for updates
library(gtbreport)                      # formatting devtools::install_github("GTB-TME/gtbreport")
...
AA5 <- data.table(a5 = c("0-4","5-9","10-14","15-19","20-24","25-29",
                         "30-34","35-39","40-44","45-49","50-54","55-59",
                         "60-64","65-69","70-74","75-79","80+"),
                  atb=c("0-4","5-14","5-14","15-24","15-24","25-34",
                        "25-34","35-44","35-44","45-54","45-54","55-64",
                        "55-64","65plus","65plus","65plus","65plus"))
...
muplot <- function(indat, cnm) {
  ## safety:
  if (all(indat$mort == 0)) {
    indat$mort <- rep(1 / nrow(indat), nrow(indat))
    cnm <- paste0(cnm,": WARNING! mort==0!")
  }
  if ("age" %ni% names(indat)) indat[, age := gsub("_", "-", age_group)]
  indat[age == "65plus", age := rev(agz4)[1]]
  indat$age <- factor(indat$age, levels = agzmc4, ordered = TRUE)
